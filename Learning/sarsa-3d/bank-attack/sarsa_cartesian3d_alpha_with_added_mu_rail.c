...
int main(int argc, char *argv[]){
    ...
    out = fopen("out3dmu_2.txt", "w");
    rew = fopen("crewards3dmu_2.txt", "w");
    Q_mat = fopen("cQ_matrix3dmu_2.txt", "w");
    Q_mat_count = fopen("cQ_counter3dmu_2.txt", "w");
    infos = fopen("infos3dmu_2.txt", "w");

    fprintf(rew, "episode,epsilon,Alpha,steps,decision_time,return,final_alpha,final_mu\n");
    fprintf(out, "epsiode,step,x_kite,y_kite,z_kite,x_block,y_block,z_block,windx,windy,windz,v_blockx,v_blocky,alpha,alpha_action,mu,mu_action,reward,Q_value\n");
    ...
    while (episode < learning_episodes){
        ...
        select_action(epsilon, Q, s_alpha, &a_alpha, s_bank, &a_bank);
        ...
        while (rk[2] > 0){
            ...
            if ( (episode%save_matrix_step == 0 || episode == learning_episodes-1) && it%decision_time == 0){
                ...
                fprintf(out, "%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%d,%f,%f\n", episode, it, rk[0], rk[1], rk[2],
                    r_block[0], r_block[1], r_block[2], W[0], W[1], W[2], v_block[0], v_block[1], alphas[s_alpha], a_alpha, bank[s_bank], a_bank, reward, \
            ...
            if (it%decision_time == 0){
                ...
                if (it == max_steps){
                    ...
                    if (episode%100 == 0){
                        printf("%f, %f, %f\n", Q[s_bank*n_actions*n_alphas*n_actions + a_bank*n_actions*n_alphas + s_alpha*n_actions + a_alpha],
                        reward + Gamma*Q[s_bank1*n_actions*n_actions*n_alphas + a_bank1*n_actions*n_alphas + s_alpha1*n_actions + a_alpha1] \
                        - Q[s_bank*n_actions*n_alphas*n_actions + a_bank*n_actions*n_alphas + s_alpha*n_actions + a_alpha],
                        Q[s_bank*n_actions*n_alphas*n_actions + a_bank*n_actions*n_alphas + s_alpha*n_actions + a_alpha]/(reward + Gamma*Q[s_bank1*n_actions*n_actions*n_alphas + a_bank1*n_actions*n_alphas + s_alpha1*n_actions + a_alpha1] \
                        - Q[s_bank*n_actions*n_alphas*n_actions + a_bank*n_actions*n_alphas + s_alpha*n_actions + a_alpha]));
                    }
