...
int main(int argc, char *argv[]){
    ...
    FILE *out ,*rew, *Q_mat, *Q_mat_count;
    out = fopen("cout_str.txt", "w");
    rew = fopen("crewards_str.txt", "w");
    Q_mat = fopen("cQ_matrix_str.txt", "w");
    Q_mat_count = fopen("cQ_counter_str.txt", "w");

    fprintf(rew, "episode,epsilon,Alpha,steps,return,penalty\n");
    fprintf(out, "episode,step,x_kite,z_kite,x_block,z_block,theta,vtheta,windx,windy,v_block,Tension,alpha,action,reward,Q[s+0],Q[s+1],Q[s+2]\n");
    ...
    while (episode < learning_episodes){
        ...
        while (rk[1] > 0){
            ...
            if ((episode%save_matrix_step == 0 || episode == learning_episodes-1) && it%decision_time == 0){
                fprintf(out, "%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%f,%f,%f\n", episode, it, rk[0], rk[1], 
                    r_block[0], r_block[1], theta, vtheta, W[0], W[1], v_block[0], T, alphas[s_alpha], a_alpha, reward, \
                    Q[s_alpha*n_actions + 0], \
                    Q[s_alpha*n_actions + 1], \
                    Q[s_alpha*n_actions + 2]);
            }   
            ...
            if (it%decision_time == 0){
                ...
                if (episode%100 == 0){
                    ...
                    reward + PENALTY - Q[s_alpha*n_actions + a_alpha],
                    Q[s_alpha*n_actions + a_alpha]/(reward + PENALTY - Q[s_alpha*n_actions + a_alpha]));
                }
