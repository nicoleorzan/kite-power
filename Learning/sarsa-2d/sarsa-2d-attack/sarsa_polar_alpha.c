...
int main(int argc, char *argv[]){
    ...
    out = fopen("pout_newcoeff_no_fixed_et.txt", "w");
    rew = fopen("prewards_newcoeff_no_fixed_et.txt", "w");
    Q_mat = fopen("pQ_matrix_newcoeff_no_fixed_et.txt", "w");
    Q_mat_count = fopen("pQ_counter_newcoeff_no_fixed_et.txt", "w");
    policy = fopen("ppolicy_newcoeff_no_fixed_et.txt", "w");
    ...
    while (episode < learning_episodes){
        ...
        if (episode == (int)learning_episodes - 20000){
            epsilon = 0.0001;
        }
        if (episode == (int)learning_episodes - 1){
            epsilon = 0.0;
        ...
        integration_trajectory(rk, vk, ak, r_block, v_block, a_block, theta, s_alpha, \
                             W, &lift, &drag, &T, &F_attr, it, &sector, &et_val);
    ...
    printf("save matrix\n");
    //save_matrix(Q, "pQ_matrix_streamfunction2.dat");
    //printf("load matrix\n");
